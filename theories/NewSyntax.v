...
(*Arguments Logic.eq_refl {A x}, {A} x.*)
Section endpoint.
...
| ERec g0 => [fset n.-1 | n in (endpoint_fv g0) & n != 0 ]
end.
...
| GBranch _ es => foldr (fun e0 acc => (gsize e0) + acc ) 1 es
| GRec  e0 => (gsize e0).+1
| _ => 1
end.
...
| EBranch _ _ es => foldr (fun e0 acc => (esize e0) + acc ) 1 es
| ERec  e0 => (esize e0).+1
| _ => 1
end.
